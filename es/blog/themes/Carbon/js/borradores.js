...
var borradores = {
	...
	printCounts: function(printCategorias){
		...
		$.each(this.counts['categorias'], function(categoria, data){
			if(printCategorias)
				$('ul#borradores-categorias').append('<li id="' + categoria + '"><p><a href="" onclick="borradores.active(this); borradores.categoria = \'' + categoria + '\'; borradores.query(); return false;">' + data['name'] + ' <span class="count">' + data['count'] + '</span></a></p></li>');
			else
	...
	active: function(e){
		$(e).parent().parent().parent().children('a').removeClass('is-active');
		$(e).parent().parent().addClass('is-active');
	},
