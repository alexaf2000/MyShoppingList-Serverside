...
function open_login_box(action){
	if($('#login_box').css('display') == 'block' && action!='open')
		...
		$('#mydialog').removeClass();
		$('#mydialog').addClass('login');
		var cuerpo = '<div id="login_box">'
        cuerpo += '<div id="login_error" class="notification is-danger has-text-centered" style="display: none;"></div>'
		cuerpo += '<form action="javascript:login_ajax()" method="post">'
		cuerpo += '<label class="label" for="nickname">Usuario</label><p class="control has-icon"><input class="input" type="text" class="ilogin" id="nickname" name="nick" maxlength="64" placeholder="Usuario"><i class="fa fa-user"></i></p>'
		cuerpo += '<label class="label" for="password">Contrase&ntilde;a</label><p class="control has-icon"><input class="input" type="password" class="ilogin" id="password" name="pass" maxlength="64" placeholder="******"><i class="fa fa-lock"></i></p>'
		cuerpo += '<p class="control hidden"><label for="rem" class="checkbox"><input type="checkbox" id="rem" name="rem" value="true" checked="checked">Remember me</label></p>'
		cuerpo += '<input type="submit" class="button is-primary is-fullwidth" value="Entrar">'
		cuerpo += '</form>'
		cuerpo += '<div class="control">'
		cuerpo += '<hr>'
		cuerpo += '<a class="label" href="#" onclick="remind_password();">&#191;Olvidaste tu contrase&#241;a?</a>'
		cuerpo += '<a class="label" href="#" onclick="resend_validation();">&#191;No lleg&oacute; el correo de validaci&oacute;n?</a>'
		cuerpo += '<p>&#191;A&uacute;n no eres miembro?</span><input class="button is-info" type="button" onclick="clogin_oregistro()" value="Reg&iacute;strate!"></p>'
		cuerpo += '</div>'
		//
		mydialog.class_aux = 'login';
		mydialog.mask_close = true;
		mydialog.close_button = true;
		mydialog.show(true);
		mydialog.title('Iniciar sesi&oacute;n');
		mydialog.body(cuerpo, 569);
...
function clogin_oregistro() {
	$('#mydialog').removeClass('LB_dialog');
	$('#mydialog').addClass('registro');
	registro_load_form();
}
...
show: function(class_aux){
	...
	if($('#mydialog').html() == '') //Primera vez
		$('#mydialog').html('<div id="dialog" class="modal"><div id="title" class="modal-card-head modal-card-title"></div><div id="cuerpo"><div id="procesando"><div id="mensaje"></div></div><div id="modalBody" class="modal-card-body"></div><div id="buttons" class="modal-card-foot"></div></div></div>');
	...
	if(this.close_button)
		$('#mydialog #dialog').append('<button class="delete close_dialog" onclick="mydialog.close()"></button>');
	else
...
buttons: function(display_all, btn1_display, btn1_val, btn1_action, btn1_enabled, btn1_focus, btn2_display, btn2_val, btn2_action, btn2_enabled, btn2_focus){
	...
	var html = '';
	if(btn1_display)
		html += '<input type="button" class="button is-primary'+(btn1_enabled?'':' disabled')+'" style="display:'+(btn1_display?'inline-block':'none')+'"'+(btn1_display?' value="'+btn1_val+'"':'')+(btn1_display?' onclick="'+btn1_action+'"':'')+(btn1_enabled?'':' disabled')+' />';
	if(btn2_display)
		html += ' <input type="button" class="button is-danger'+(btn1_enabled?'':' disabled')+'" style="display:'+(btn2_display?'inline-block':'none')+'"'+(btn2_display?' value="'+btn2_val+'"':'')+(btn2_display?' onclick="'+btn2_action+'"':'')+(btn2_enabled?'':' disabled')+' />';
	$('#mydialog #buttons').html(html).css('display', 'inline-block');

	if(btn1_focus)
		$('#mydialog #buttons .button.is-primary').focus();
	else if(btn2_focus)
		$('#mydialog #buttons .button.is-danger').focus();
},
buttons_enabled: function(btn1_enabled, btn2_enabled){
	if($('#mydialog #buttons .button.is-primary'))
		if(btn1_enabled)
			$('#mydialog #buttons .button.is-primary').removeClass('disabled').removeAttr('disabled');
		else
			$('#mydialog #buttons .button.is-primary').addClass('disabled').attr('disabled', 'disabled');

	if($('#mydialog #buttons .button.is-danger'))
		if(btn2_enabled)
			$('#mydialog #buttons .button.is-danger').removeClass('disabled').removeAttr('disabled');
		else
			$('#mydialog #buttons .button.is-danger').addClass('disabled').attr('disabled', 'disabled');
},
...
function TopsTabs(parent, tab) {
		...
		$('#'+parent+' > .box_cuerpo div.filterBy a').removeClass('is-active');
		$('.box_cuerpo div.filterBy a#'+tab).addClass('is-active');
...
var mensaje = {
    ...
    form: function (){
         var html = '';
        if(this.vars['error']) html += '<div class="notification is-danger has-text-centered">' + this.vars['error'] + '</div>'
        html += '<div class="label">Para:</div>'
        html += '<div class="control"><input class="input" type="text" value="' + this.vars['to'] + '" maxlength="16" tabindex="0" size="20" id="msg_to" name="msg_to"/> <span style="font-size: 10px;">(Ingrese el nombre de usuario)</span></div>'
        html += '<div class="label">Asunto:</div>'
        html += '<div class="control"><input class="input" type="text" value="' + this.vars['sub'] + '" maxlength="100" tabindex="0" size="50" id="msg_subject" name="msg_subject"/></div>'
        html += '<div class="label">Mensaje:</div>'
        html += '<div class="control"><textarea class="textarea" tabindex="0" rows="10" id="msg_body" name="msg_body" style="height:100px; width:350px">' + this.vars['msg'] + '</textarea></div>'
        return html;                          
    },
    // FUNCIONES AUX
...
var preview = $('#thumb_preview').html('<div class="thumbnail-img has-text-centered good"><span class="icon"> <i class="fa fa-circle-o-notch fa-spin fa-fw"></i></span><span> Cargando la imagen. Por favor espere...</span></div>');
var i = new Image();
...
preview.html('<div class="thumbnail-img has-text-centered error"> <span class="icon"><i class="fa fa-warning"></i></span><span>La imagen no se pudo cargar o no existe. Por favor reintentalo.</span></div>') & document.getElementById('thumbnail').click();
i = null;
}, 1000);
i.onload = function () {
clearTimeout(t.timeout);
