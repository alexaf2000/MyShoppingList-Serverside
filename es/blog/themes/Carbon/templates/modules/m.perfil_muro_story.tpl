                                ...
                                <div class="Story media" id="pub_{$p.pub_id}">
								<div class="media-left">
                                    <a href="{$tsConfig.url}/perfil/{$p.user_name}" class="Story_Pic">
									<figure class="image is-64x64">
									<img class="round" alt="{$p.user_name}" src="{$tsConfig.url}/files/avatar/{$p.p_user_pub}_120.jpg"/>
									</figure>
									</a>
                                </div>    
									<div class="Story_Content media-content">
                                        <div class="Story_Head">
                                            {if $p.p_user == $tsUser->uid || $p.p_user_pub == $tsUser->uid || $tsUser->is_admod || $tsUser->permisos.moepm}<div class="Story_Hide"><a href="#" onclick="muro.del_pub({$p.pub_id},1); return false;" title="Eliminar la publicaci&oacute;n" class="qtip uiClose"><i class="fa fa-times"></i></a></div>{/if}
                                            <div class="Story_Message content">
                                                <div><a href="{$tsConfig.url}/perfil/{$p.user_name}" class="a_blue"><strong>{if $p.user_name == $tsUser->nick}Yo{else}{$p.user_name}{/if}</strong></a>
												<small>{$p.p_date|fecha}</small>
												</div> 
                                                ...
                                                <li class="ufiItem" {if $p.p_likes == 0}style="display:none"{/if}>
                                                    <div class="likes clearfix">
                                                       <span class="icon is-small"> <i class="fa fa-thumbs-o-up"></i></span>
                                                        <span id="lk_{$p.pub_id}">{$p.likes.text}</span>
                                                    </div>
                                                </li>
                                                <li>
                                                   <ul id="cl_{$p.pub_id}" class="commentList">
                                                        ...
                                                        <li class="ufiItem" id="cmt_{$c.cid}">
                                                            <div class="clearfix">
                                                                ...
                                                                {if $p.p_user == $tsUser->uid || $c.c_user == $tsUser->uid  || $tsUser->is_admod || $tsUser->permisos.moecm}<span class="close"><a href="#" onclick="muro.del_pub({$c.cid}, 2); return false" class="uiClose" title="Eliminar"><i class="fa fa-times"></i></a></span>{/if}
                                                                <div class="mensaje">
