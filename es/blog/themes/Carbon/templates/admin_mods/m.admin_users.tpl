									<h3 class="title">Administrar Usuarios</h3>
								...
								<table cellpadding="0" cellspacing="0" border="0" class="admin_table table">
									<thead>
										<th>Rango</th>
										<th>Usuario</th>
										<th class="is-hidden-touch"><a class="qtip" title="Ordenar por email ascendente" href="{$tsConfig.url}/admin/users?o=c&m=a"><</a> Email <a class="qtip" title="Ordenar por email descendente" href="{$tsConfig.url}/admin/users?o=c&m=d">></a></th>
										<th class="is-hidden-touch"><a class="qtip" title="Ordenar por &uacute;ltima vez activo ascendente" href="{$tsConfig.url}/admin/users?o=u&m=a"><</a> &Uacute;ltima actividad <a class="qtip" title="Ordenar por &uacute;ltima vez activo desccendente" href="{$tsConfig.url}/admin/users?o=u&m=d">></a></th>
										<th class="is-hidden-touch">Registro</th>
										<th class="is-hidden-touch"><a class="qtip" title="Ordenar por IP ascendente" href="{$tsConfig.url}/admin/users?o=i&m=a"><</a> IP <a class="qtip" title="Ordenar por IP descendente" href="{$tsConfig.url}/admin/users?o=i&m=d">></a> </th>
										<th class="is-hidden-touch"><a class="qtip" title="Ordenar por estado ascendente" href="{$tsConfig.url}/admin/users?o=e&m=a"><</a> Estado <a class="qtip" title="Ordenar por estado descendente" href="{$tsConfig.url}/admin/users?o=e&m=d">></a></th>
										<th>Acciones</th>
									</thead>
									<tbody>
										...
										<tr>
											...
											<td class="is-hidden-touch">{$m.user_email}</td>
											<td class="is-hidden-touch">{if $m.user_lastactive == 0} Nunca{else}{$m.user_lastactive|hace:true}{/if}</td>
											<td class="is-hidden-touch">{$m.user_registro|date_format:"%d/%m/%Y"}</td>
											<td class="is-hidden-touch"><a href="{$tsConfig.url}/moderacion/buscador/1/1/{$m.user_last_ip}" class="geoip" target="_blank">{$m.user_last_ip}</a></td>
											<td class="is-hidden-touch" id="status_user_{$m.user_id}">{if $m.user_baneado == 1}<font color="red">Suspendido</font>{elseif $m.user_activo == 0}<font color="purple">Inactivo</font>{else}<font color="green">Activo</font>{/if}</td>
											<td class="admin_actions">
												<a href="{$tsConfig.url}/admin/users?act=show&uid={$m.user_id}"><span class="icon"><i class="fa fa-edit" title="Editar Usuario"></i></span></a>												
												<a onclick="admin.users.setInActive({$m.user_id}); return false;"><span class="icon"><i class="fa fa-refresh" title="Activar/Desactivar Usuario"></i></span></a>
                                                <a href="#" onclick="mod.users.action({$m.user_id}, 'aviso', false); return false;"><span class="icon"><i class="fa fa-warning" title="Enviar Alerta"></i></span></a>
												<a href="#" onclick="mod.{if $m.user_baneado == 1}reboot({$m.user_id}, 'users', 'unban', false){else}users.action({$m.user_id}, 'ban', false){/if}; return false;"><span class="icon"><i class="fa fa-ban" title="{if $m.user_baneado == 1}Reactivar{else}Suspender{/if} Usuario"></i></span></a>
											</td>
										</tr>
										{/foreach}
									</tbody>
								...
								<form action="" method="post">
									...
									{if !$tsType || $tsType == 1}
										...
										<dl>
											...
											<dd><input type="text" name="nick" id="user" value="{$tsUserD.user_name}" class="qtip input" title="El nick s&oacute;lo se cambiar&aacute; si escribe una nueva contrase&ntilde;a" /></dd>
										</dl>
										<dl>
										...
										<dl>
											...
											<dd><input class="input" type="text" name="points" id="points" value="{$tsUserD.user_puntos}" /></dd>
										</dl>
										<dl>
											<dt><label>Puntos para dar:</label></dt>
											<dd><input class="input" type="text" name="pointsxdar" id="pointsxdar" value="{$tsUserD.user_puntosxdar}"  /></dd>
										</dl>
										<dl>
											<dt><label>Cambios de nick disponibles:</label></dt>
											<dd><input class="input" type="text" name="changenicks" id="changenicks" value="{$tsUserD.user_name_changes}" /></dd>
										</dl>
										<hr />
										<dl>
											<dt><label for="email">E-mail:</label></dt>
											<dd><input class="input" type="text" name="email" id="email" value="{$tsUserD.user_email}" /></dd>
										</dl>
										<dl>
											<dt><label for="pwd">Nueva contrase&ntilde;a:</label><br /><span>Debe tener entre 5 y 35 caracteres.</span></dt>
											<dd><input class="input" type="password" name="pwd" id="pwd" onkeypress="if($('#cpwd').val() != '') $('#sendata').fadeIn();"/></dd>
										</dl>
										<dl>
											...
											<dd><input class="input" type="password" name="cpwd" id="cpwd" onkeypress="if($('#pwd').val() != '') $('#sendata').fadeIn();"/></dd>
										</dl>
									...
									<legend>Eliminaci&oacute;n de contenidos</legend>
                                        ...
                                        <br /><hr/>
                                        Introduzca su contrase&ntilde;a para continuar: <input class="input" type="password" name="password"/>
									...
									<textarea class="textarea" name="firma" rows="3" cols="50">{$tsUserF.user_firma}</textarea>
									{else}
									<div class="phpostAlfa">Pendiente</div>
									{/if}
									<p><input type="submit" name="save" value="Enviar Cambios" class="button is-primary"/></p>
									</fieldset>
								</form>
								{/if}
								</div>