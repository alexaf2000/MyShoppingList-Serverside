                                ...
                                <h3 class="title">Administrar Rangos de Usuarios</h3><hr/>
                                ...
                                {if $tsAct == ''}
                                <div>
                                <h3 class="title is-5">Rangos Especiales</h3><hr/>
                                <table cellpadding="0" cellspacing="0" border="0" class="admin_table table">
                                	<thead>
                                    	<th>Rango</th>
                                        <th class="is-hidden-touch">Usuarios</th>
                                        <th class="is-hidden-touch">Puntos para dar</th>
										<th class="is-hidden-touch">Puntos por post</th>
                                        <th>Acciones</th>
                                    </thead>
                                    <tbody>
                                    {foreach from=$tsRangos.regular item=r}
                                    	<tr>
                                        	<td><a href="?act=list&rid={$r.id}&t=r" style="color:#{$r.color}"><img src="{$tsConfig.url}/themes/default/images/icons/ran/{$r.imagen}" /> {$r.name}</a></td>
                                            <td class="is-hidden-touch">{$r.num_members}</td>
                                            <td class="is-hidden-touch">{$r.user_puntos}</td>
											<td class="is-hidden-touch">{$r.max_points}</td>
                                            <td class="admin_actions">
                                            <a href="?act=editar&rid={$r.id}&t=s"><span class="icon is-small qtip" title="Editar Rango"><i class="fa fa-edit"></i></span></a>
                                            {if $r.id > 3}<a href="?act=borrar&rid={$r.id}" ><span class="icon is-small qtip" title="Borrar Rango"><i class="fa fa-trash"></i></span></a>
											{/if}{if $tsConfig.c_reg_rango == $r.id}<span  class="icon is-small qtip" title="Rango Predeterminado al registro"><i class="fa fa-check-square"></i></span>{else}<span id="f_2" onclick="location.href = '{$tsConfig.url}/admin/rangos/?act=setdefault&rid={$r.id}'; $('#f_2').style.cursor=wait"  class="icon is-small qtip" title="Establecer Predeterminado" style="cursor:pointer;"><i class="fa fa-refresh"></i></span>{/if}
                                            </td>
                                        </tr>
                                    {/foreach}
                                    </tbody>
                                    <tfoot>
                                    	<td colspan="6" style="text-align:right"><a class="button is-primary" href="?act=nuevo&t=s">Agregar nuevo rango &raquo;</a></td>
                                    </tfoot>
                                </table>
                                </div>
                                <div>
                                <h3 class="title is-5">Rangos basados en el conteo de puntos y posts</h3><hr />
                                <table cellpadding="0" cellspacing="0" border="0" class="admin_table table">
                                	<thead>
                                    	<th>Rango</th>
                                        <th class="is-hidden-touch">Usuarios</th>
                                        <th>Tipo</th>
                                        <th class="is-hidden-touch">Cantidad requerida</th>
                                        <th class="is-hidden-touch">Puntos para dar</th>
										<th class="is-hidden-touch">Puntos por post</th>
                                        <th>Acciones</th>
                                    </thead>
                                    ...
                                    {foreach from=$tsRangos.post item=r}
                                    	<tr>
                                        	<td><a href="?act=list&rid={$r.id}&t=p" style="color:#{$r.color}"><img src="{$tsConfig.url}/themes/default/images/icons/ran/{$r.imagen}" /> {$r.name}</a></td>
                                            <td class="is-hidden-touch">{$r.num_members}</td>
                                            <td>{if $r.type == 1}Puntos{elseif $r.type == 2}Posts{elseif $r.type == 3}Fotos{elseif $r.type == 4}Comentarios{/if}</td>
                                            <td class="is-hidden-touch">{$r.cant}</td>
                                            <td class="is-hidden-touch">{$r.user_puntos}</td>
											<td class="is-hidden-touch">{$r.max_points}</td>
                                            <td class="admin_actions">
                                            <a href="?act=editar&rid={$r.id}&t=p"><span class="icon is-small qtip" title="Editar Rango"><i class="fa fa-edit"></i></span></a>
                                            {if $r.id > 3}<a href="?act=borrar&rid={$r.id}"><span class="icon is-small qtip" title="Borrar Rango"><i class="fa fa-trash"></i></span></a>{/if}
                                    ...
                                    <tfoot>
                                    	<td colspan="8" style="text-align:right"><a class="button is-dark" href="?act=nuevo">Agregar nuevo rango &raquo;</a></td>
                                    </tfoot>
                                </table>
                                </div>
                                {elseif $tsAct == 'list'}
                                ...
                                {literal}
                                <style>
								#colores {width:200px; position:absolute; z-index: 2; right:50px; padding:15px 8px 10px 10px; border:1px solid #ccc; background-color:#fafafa;}
								#cerrar {position:absolute; right:5px; top:3px; z-index:2}
								#colores .title {position:absolute; left:10px; top:0px; z-index:2; font-weight:bold; font-size: 11px!important;}
                                ...
                                <fieldset>
                                    <div id="colores"><span class="title">Colores</span><a href="#" id="cerrar"><i class="fa fa-times-circle" aria-hidden="true"></i></a></div>
                                    <legend>Nuevo Rango</legend>
				
				<input type="button" id="b1" onclick="$('#tab1').fadeIn('slow'); $('#tab2').hide('slow'); $(this).css('border','1px solid #4D90FE'); $('#b2').css('border','0px');" value="B&aacute;sico" style="width:100px;cursor:default;border: 1px solid #4D90FE;" class="button is-primary"/> 
				
				<input type="button" id="b2" onclick="$('#tab2').show('slow'); $('#tab1').hide('slow'); $(this).css('border','1px solid #4D90FE'); $('#b1').css('border','0px');" value="Permisos" style="width:100px;cursor:default;" class="button is-info"/> 
									
									<div id="tab1">
									
                                    <dl>
                                        <dt><label for="rName">T&iacute;tulo:</label></dt>
                                        <dd><input class="input" type="text" id="rName" name="rName" value="{$tsRango.r_name}" style="width:100%"/></dd>
                                    </dl>
                                	<dl>
                                        ...
                                        <dd><input class="input" type="text" id="rColor" name="rColor" value="{$tsRango.r_color}" style="color:#{$tsRango.r_color}; font-weight:bold;width:100%"/></dd>
                                    </dl>
									<dl>
                                        ...
                                        <dd><input class="input" type="text" id="gopfd" name="global-pointsforday" value="{$tsRango.permisos.gopfd}" style="width:100%"/></dd>
                                    </dl>
									<dl>
                                        <dt><label for="gopfp">Puntos por post</label><br /><span>Puntos que puede dar en cada post.</span></dt>
                                        <dd><input class="input" type="text" id="gopfp" name="global-pointsforposts" value="{$tsRango.permisos.gopfp}" style="width:100%"/></dd>
                                    </dl>
									<dl>
                                        <dt><label for="goaf">Anti-flood</label><br /><span>Tiempo que deben esperar entre acci&oacute;n.</span></dt>
                                        <dd><input class="input" type="text" id="goaf" name="global-antiflood" value="{$tsRango.permisos.goaf}" style="width:100%"/></dd>
                                    </dl>
									<dl>
										...
										<dd>
											...
											<input class="input" type="text" id="gocpr" name="global-cantidadrequerida" style="width:50%{if $tsRango.r_type == 0};display:none;{/if}" maxlength="5" value="{$tsRango.r_cant}" />
                                    ...
                                    <dl>
                                        ...
                                        <dd>
                                            ...
                                            <div class="select">
											<select name="r_img" id="cat_img" style="width:100%">
                                    ...
                                    <hr />
									<input type="button" onclick="$('#tab2').show('slow'); $('#tab1').hide('slow'); $('#b2').css('border','1px solid #4D90FE'); $('#b1').css('border','0px');" value="Continuar" class="button is-info"/> 
									</div>									
                                ...
                                <form action="" method="post" id="admin_form">
                                	<div class="notification is-danger has-text-centered">Si borras este rango todos los usuarios que est&eacute;n en &eacute;l, ser&aacute;n asignados al rango 
									...
									<label>&nbsp;</label> <input type="submit" name="save" value="S&iacute;, Continuar &raquo;" class="button is-danger">
                                </form>
                                {/if}
                                </div>